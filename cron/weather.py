...
class GetWeather:
    def __init__(self):
        ...
        current_path = "D:\\CITY.txt"
        with open(current_path , 'r', encoding='UTF-8') as load_f:
...
class dbUtil():
    ...
    def get_conn(self):
        # 建立连接
        conn = pymysql.connect(host="127.0.0.1", port=3306, user="root", password="123456", db="weathers",
                               charset="utf8")
        # c创建游标A
        cursor = conn.cursor()
        return conn, cursor
